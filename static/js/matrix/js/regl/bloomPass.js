...
export default ({ regl, config }, inputs) => {
	...
	const highPassFrag = loadText("/js/matrix/shaders/glsl/bloomPass.highPass.frag.glsl");
	const highPass = regl({
		frag: regl.prop("frag"),
		uniforms: {
			highPassThreshold,
	...
	const blurFrag = loadText("/js/matrix/shaders/glsl/bloomPass.blur.frag.glsl");
	const blur = regl({
		frag: regl.prop("frag"),
		uniforms: {
			tex: regl.prop("tex"),
	...
	const combineFrag = loadText("/js/matrix/shaders/glsl/bloomPass.combine.frag.glsl");
	const combine = regl({
		frag: regl.prop("frag"),
		uniforms: {
			bloomStrength,
